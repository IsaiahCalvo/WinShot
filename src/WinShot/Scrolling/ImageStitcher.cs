...
public static class ImageStitcher
{
    ...
    public static int FindScrollOffset(SD.Bitmap previous, SD.Bitmap current)
    {
        ...
        // Rows whose hash appears only once within the frame are "distinctive"; rows whose
        // hash repeats (blank/whitespace runs) are not. We require enough distinctive
        // matches so a whitespace band can't carry an offset.
        bool[] currDistinctive = MarkDistinctiveRows(currHashes, 0, height);
        ...
        {
            // Compare current row y against previous row y+offset (y+offset must stay
            // inside the frame).
