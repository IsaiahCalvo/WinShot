...
- **Annotation editor hierarchy, accessibility, shortcuts, and project fidelity** - the local editor now follows the verified primary tool order with WinShot-only tools in More, exposes immediate per-tool controls, gives icon controls explicit automation help and focus rings, makes Ctrl+C copy, and preserves arrow and letter-counter styles through project reload. The emoji creation tool and bottom-bar Drag Me control were intentionally removed at the product owner's request; legacy emoji project data remains readable.
...
- **Curved arrows, text styles, and filled shapes** - implemented; the editor includes curved arrows with adjustable handles, text styles, and rectangle/ellipse fill modes, with live drawing and project reload now sharing tested fill-brush rules.
